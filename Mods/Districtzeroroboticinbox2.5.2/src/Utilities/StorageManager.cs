...
namespace RoboticInbox.Utilities
{
    internal class StorageManager
    {
        ...
        internal static void Distribute(Vector3i sourcePos)
        {
            _log.Trace($"Distribute called for tile entity at {sourcePos}");
            var world = GameManager.Instance.World;
            var source = world.GetTileEntity(sourcePos);
            ...
            ActiveCoroutines.Add(sourcePos, ThreadManager.StartCoroutine(OrganizeCoroutine(world, sourcePos, source, sourceContainer, min, max)));
        }
        ...
        private static IEnumerator OrganizeCoroutine(World world, Vector3i sourcePos, TileEntity source, ITileEntityLootable sourceContainer, Vector3i min, Vector3i max)
        {
            ...
            for (var distance = 1; distance <= maxDist; distance++)
            {
                ...
                if (sourcePos.y - distance >= min.y)
                {
                    ...
                    for (var x = Utils.FastMax(sourcePos.x - distance, min.x); x <= Utils.FastMin(sourcePos.x + distance, max.x); x++)
                    {
                        ...
                        {
                            if (VerifyContainer(world, x, y, z, out var targetPos, out var target, out var targetContainer))
                            {
        ...
        private static bool VerifyContainer(World world, int x, int y, int z, out Vector3i pos, out TileEntity tileEntity, out ITileEntityLootable tileEntityLootContainer)
        {
        ...
        private static bool CheckAndHandleInUse(ITileEntityLootable sourceContainer, Vector3i sourcePos, ITileEntityLootable targetContainer, Vector3i targetPos)
        {
            if (LockManager.Instance.IsLockedServer(sourceContainer))
            {
                _log.Trace($"a player is currently accessing source container at {sourcePos}; skipping");
                NotificationManager.PlaySoundVehicleStorageOpen(sourcePos);
                return true;
            }
            if (LockManager.Instance.IsLockedServer(targetContainer))
            {
                _log.Trace($"a player is currently accessing target container at {targetPos}; skipping");
                NotificationManager.NotifyInUse(targetPos);
                return true;
            }
            return false;
        }
        ...
        {
            if (CheckAndHandleInUse(sourceContainer, sourcePos, targetContainer, targetPos))
            {
            ...
            try
            {
                ...
                for (var s = 0; s < sourceContainer.items.Length; s++)
                {
                    ...
                    // Skip locked slots in the source container
                    if (sourceContainer.HasSlotLocksSupport && sourceContainer.SlotLocks != null && sourceContainer.SlotLocks[s])
                    {
                        continue;
                    }
        ...
        private static bool CanAccess(TileEntity source, TileEntity target, Vector3i targetPos)
        {
            ...
            if (sourceLock.GetPasswordHash().Equals(targetLock.GetPasswordHash()))
            {
                return true;
            }

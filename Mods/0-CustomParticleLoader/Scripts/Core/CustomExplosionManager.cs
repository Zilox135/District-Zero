...
public static class CustomExplosionManager
{
    ...
    internal static void GetValRaw(DynamicProperties p, string k, ref string v)
    {
        if (p.Values.TryGetValue(k, out var s)) v = s;
    }

    internal static void GetValRaw(DynamicProperties p, string k, ref bool v)
    {
        if (p.Values.TryGetValue(k, out var s) && StringParsers.TryParseBool(s, out var r)) v = r;
    }

    internal static void GetValRaw(DynamicProperties p, string k, ref float v)
    {
        if (p.Values.TryGetValue(k, out var s) && StringParsers.TryParseFloat(s, out var r)) v = r;
    }
    ...
    public static bool parseParticleData(DynamicProperties _props, out ExplosionComponent component)
    {
        ...
        component = null;
        GetValRaw(_props, "Explosion.ParticleIndex", ref str_index);
        if (str_index != null && str_index.StartsWith("#"))
        {
            ...
            GetValRaw(_props, "Explosion.Overwrite", ref overwrite);
            if (overwrite || !GetCustomParticleComponents(hashed_index, out _))
            {
                string sound_name = null;
                GetValRaw(_props, "Explosion.AudioName", ref sound_name);
                float duration_audio = -1;
                GetValRaw(_props, "Explosion.AudioDuration", ref duration_audio);
                bool sync = false;
                GetValRaw(_props, "Explosion.SyncOnConnect", ref sync);
                bool observe = false;
                GetValRaw(_props, "Explosion.IsChunkObserver", ref observe);
                _props.Values.TryGetValue("Explosion.CustomScriptTypes", out string customScriptTypes);
                getTypeListFromString(customScriptTypes, out List<Type> list_customtypes);

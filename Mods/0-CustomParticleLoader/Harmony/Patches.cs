...
internal class ExplosionEffectPatch
{
    ...
    [HarmonyPrefix]
    private static bool explode_Prefix(GameManager __instance, Vector3 _worldPos, Vector3i _blockPos, Quaternion _rotation, ExplosionData _explosionData, int _entityId, ItemValue _itemValueExplosionSource, out ExplodeState __state)
    {
        ...
        if (index >= WorldStaticData.prefabExplosions.Length)
        {
            ...
            if (flag && components != null)
            {
                ...
                ExplosionValue value = new ExplosionValue()
                {
                    ...
                    CurrentExplosionParams = new ExplosionParams(0, _worldPos, _blockPos, _rotation, _explosionData, _entityId, CustomExplosionManager.NextExplosionIndex++),
    ...
    private static IEnumerable<CodeInstruction> explode_Transpiler(IEnumerable<CodeInstruction> instructions)
    {
        ...
        MethodInfo mtd_explosion_client = AccessTools.Method(typeof(GameManager), nameof(GameManager.ExplosionClient));

        CodeInstruction ldlocResult = null;
        for (int j = 0; j < codes.Count - 1; j++)
        {
            if (codes[j].Calls(mtd_explosion_client))
            {
                var st = codes[j + 1];
                if (st.opcode == OpCodes.Stloc_0) ldlocResult = new CodeInstruction(OpCodes.Ldloc_0);
                else if (st.opcode == OpCodes.Stloc_1) ldlocResult = new CodeInstruction(OpCodes.Ldloc_1);
                else if (st.opcode == OpCodes.Stloc_2) ldlocResult = new CodeInstruction(OpCodes.Ldloc_2);
                else if (st.opcode == OpCodes.Stloc_3) ldlocResult = new CodeInstruction(OpCodes.Ldloc_3);
                else if (st.opcode == OpCodes.Stloc_S) ldlocResult = new CodeInstruction(OpCodes.Ldloc_S, st.operand);
                else if (st.opcode == OpCodes.Stloc) ldlocResult = new CodeInstruction(OpCodes.Ldloc, st.operand);
                break;
            }
        }
        if (ldlocResult == null) ldlocResult = new CodeInstruction(OpCodes.Ldnull);
        ...
        for (int i = 0, totali = codes.Count; i < totali; i++)
        {
            if (codes[i].opcode == OpCodes.Callvirt && codes[i].Calls(mtd_sendpackage))
            {
                ...
                for (int k = i - 1; k >= 0; k--)
                {
                    if ((codes[k].opcode == OpCodes.Call || codes[k].opcode == OpCodes.Callvirt)
                        && codes[k].operand is MethodInfo mi
                        && mi.Name == "get_Instance"
                        && typeof(ConnectionManager).IsAssignableFrom(mi.ReturnType))
                    {
                        startIdx = k;
                        break;
                    }
                ...
                if (startIdx >= 0)
                {
                    codes.RemoveRange(startIdx, i - startIdx + 1);
                }
                break;
            }
        }
...
internal class EntityExplosionPatch
{
    private static void Postfix(EntityAlive __instance)
    {
        ...
        if (explosion.ParticleIndex > 0)
            GameManager.Instance.ExplosionServer(__instance.GetPosition(), World.worldToBlockPos(__instance.GetPosition()), Quaternion.identity, explosion, __instance.entityId, 0f, false, null);
    }
}
